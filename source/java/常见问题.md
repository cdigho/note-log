...
![1736067469159](assets\1736067469159.png)
  ...
  ```java
      ...
      public static void main(String[] args) throws InterruptedException {
          ...
          Thread writerThread = new Thread(() -> {
              try {
                  Thread.sleep(1000); // 模拟一些计算或延迟
              } catch (InterruptedException e) {
                  Thread.currentThread().interrupt();
              }
              flag = true;
              System.out.println("Writer thread has updated the flag to: " + flag);
          });
  
          // 创建一个线程，它会不断检查flag的值
          Thread readerThread = new Thread(() -> {
              while (!flag) {
                  // 由于内存一致性问题的存在，即使flag被writerThread设置为true，
                  // readerThread可能仍然读取到旧的值false
              }
              System.out.println("Reader thread has detected the flag is: " + flag);
          });
...
![集合概述](assets/20210602173658349.png)
...
import java.util.Map;
    /**
     * 错误示例会抛出ConcurrentModificationException异常
     */
    public static void main(String[] args) {
...
在 JDK1.7 版本中，ConcurrentHashMap 由数组 + Segment + 分段锁实现，其内部分为多个段（Segment）的数组，Segment 通过继承 ReentrantLock 来进行加锁，通过每次锁住一个 segment 来降低锁的粒度(哈希表被分成多段)而且保证了每个 segment 内的操作的线程安全性，从而实现全局线程安全。下图就是 JDK1.7 版本中 ConcurrentHashMap 的结构示意图：
...
```
HashMap在扩容时候存在线程安全死循环，扩容时头插法会在新map中添加的顺序和原来的相反，但其它线程拿到的是旧顺序，造成链表相互引用。
...
头插法在扩容转移过程中
线程1正常扩容执行完成  A->B->C 由于头插法顺序相反扩容转移变为  C->B->A
        ...
        if (threadPool != null) {
            ...
            synchronized (ThreadPoolUtil.class) {
                if (threadPool == null) {
                     ...
                     * 空闲状态保留的线程数
                     * 最大线程数
                     * 最大线程数相比核心线程数的剩余线程等待新任务的时间以及单位
                     * 新任务被提交后，会先进入到此工作队列中，任务调度时再从队列中取出任务
                     * 达到线程边界或队列容量而阻塞的时候使用的处理程序
                     */
...
使用必须在synchronized修饰的代码块或者方法中，作用的都是当前对象上的线程。wait锁定了一个线程，必须要通过其他的已经获取锁的线程调用notify方法去唤醒。
...
说白了其实就是在使用对象的时候标记了引用，所以在内存回收的时候可以更加高效的得知对象的使用情况不必暂停整个应用程序。
```
...
- 拆分策略
  ...
  - 映射表，单独的表存储映射关系
- ID唯一性
  - GUID Google对uuid的实现， 用于在拥有多个节点、多台计算机的网络或系统中。 
  - 为每个分片指定一个 ID 范围
  - 分布式 ID 生成器 (如 Twitter 的 Snowflake 算法)
  - 我理解ID可以使用雪花ID加分片ID的组合(1010000001)，101代表第一个分片后面跟雪花ID



## mysql

### mysql底层数据结构是什么？做过哪些数据库优化？
...
### mysql 底层b+树存储在磁盘上，那索引同时存储在磁盘时为什么索引会速度快呢？磁盘读取很慢
...
下面两个事务中，执行完前两句后在执行第三句SQL将发生相互持有锁未提交的情况就会发生死锁。
事务一
begin;
UPDATE  goods  set num=num-1 WHERE id=1;
UPDATE  goods  set num=num-1 WHERE id=2;
commit;
...
流程生成一个readview(快照读) 由undolog记录日志 隐式字段记录事务属性 
```

### 存储引擎

### InnoDB 索引方式 聚合索引 索引原理 实现方式
...
innodb受益于mvcc多版本并发控制发生了写锁也可以读取
```
...
tinyint int bigint 在于存储大小和所占字节大小的不同
...
```
关注Type、Key、Key_len、Rows和Extra这几个字段
Type应该是const、eq_ref、ref或range，这些类型通常意味着较快的查询速度。
key使用的索引名称
key_len索引的长度越小越好
rows查询的行数越小越好
Extra描述一些额外信息例如
...
幻读：一个事务查询的前后结果，读取到另一个事务插入的数据

不可重复读：一个事务查询的前后结果，读取到另一个事务修改的数据
...
- Read committed 读提交
  ...
  读取另一个事务提交的数据，此刻读取到的数据，在未提交之前可能受其它事务影响(insert/update/delete)二次读取到的结果不一致。可以解决脏读问题，但不能解决不可重复读问题和幻读问题。 Sql Server , Oracle默认
- Repeatable read 可重复度

  同一个事务多次读取的结果是一致的。重复读可以解决不可重复读问题。但是可能还会有幻读问题。幻读问题对应的是插入insert操作。MySQL默认
- Serializable 串行化
...
- 驳斥二观点：CPU切换不会造成CPU升高
  从CPU 时间片是什么问题中,描述时间片会公平地分配CPU资源也就是说过多的线程会更加消耗资源用于下文切换。
- ChatGPT
...
### [Redis集群方案](https://www.cnblogs.com/51life/p/10233340.html)

三种集群模式实际上是自身的不断升级优化。

主从模式 主节点向从节点同步数据，但是宕机需要手动重启（解决了负载和数据同步问题）

哨兵模式 自动化的系统监控和故障恢复功能,每个节点存储相同数据浪费内存（解决了自动故障恢复问题）

集群模式 已自身的算法将数据存储在不同的槽中（解决了自动故障恢复问题和数据分布式存储能力），当主节点挂点之后，会向其他的主节点申请成为主节点，票数需要是总的节点票数的一半也上，也就是说集群节点最少三个主从结构。

哪一个节点可以作为主节点？运行状态正常的节点，节点中数据最新的节点，多个数据最新相对id最小的节点。

网络延迟出现了多个主节点脑裂？可以设置主从链接的延迟时间，相对长一点网络抖动就会结束。网络恢复后一般最新版本的节点会作为主节点。

如果redis集群中一个主从结构全部挂了，这个redis集群还可以使用吗？一般情况剩余的主从节点可以满足最小运行要求就可以运行。

![在这里插入图片描述](assets/a3b81acb61e5c4348e92508b58cd90c7.png)
...
[rediscluster动态扩容](https://developer.aliyun.com/article/574045?spm=5176.21213303.J_qCOwPWspKEuWcmp8qiZNQ.146.15972f3d25B2LA&scm=20140722.S_community@@%E6%96%87%E7%AB%A0@@574045._.ID_community@@%E6%96%87%E7%AB%A0@@574045-RL_redis%E6%89%A9%E5%AE%B9-LOC_llm-OR_ser-V_3-RE_new2-P0_15)

[阿里云云产品自动扩容](https://help.aliyun.com/zh/redis/user-guide/enable-automatic-scale-up?spm=5176.28525400.J_2d9ImNIIBgRqiC15YITY3.2.4111482bEyU10N#task-2057629)

看下面阿里云的解释应该是没有完美方案的尤其是数据量访问量大的话，长时间的数据同步并且提供可用实例不太现实。

![image-20240511175228803](assets/image-20240511175228803.png)
...
ziplist:压缩列表,是一种连续分配的内存结构
Quicklist：快速链表当元素过多的时候会由多个ziplist构成
```
...
3. 公平锁（Fair Lock）：Redisson的公平锁确保锁的获取按照线程的请求顺序进行，即先到先得的原则。
...
优势：可靠性高（红锁）、可以自动续期、lua脚本可以保证一组命令的原子性（多个客户端的多个命令可能存在先后顺序问题）

自实现缺点：无法续期、节点故障、同一个线程的锁重入问题、是否是同一个线程的锁释放问题
...
logs 容器日志  docker logs -f --tail=30  8d96de00f42d

exec  执行容器命令  docker  exec 788cd1a4b8ed ls /

# Kubernetes

一般通过kubectl客户端工具连接

 get pods 查找pod kubectl get pods --all-namespaces|grep pod-name

logs 日志 logs -f pod-name -n your-namespace

exec 执行命令 exec -ti pod-name -n your-namespace  -- ls

进入pod exec -ti your-pod-name -n your-namespace  -- /bin/sh

cp 拷贝文件 cp -n namespace -c container  podName:filePath localFilePath

inres(nginx )

svc：service的缩写相当于一组pod的负载均衡
...
pwd、cd、mkdir、chmod、rm、cp、mv、cat、tail -f、vim、kill、ping、curl、ssh、history
...
- `消息标识（MessageId/Key）` 由用户指定的业务相关的唯一标识(orderId) 该属性能决定消息发送到哪一个队列。顺序消息。
- `消费者分组（ConsumerGroup）` 承载多个消费行为一致的消费者的负载均衡分组。同一个分组可以使用顺序消息。

### 顺序消息

- 保证投递者顺序发送官网中需要指定消费分组
...
- 指的是在一个分布式系统中，Consistency（一致性）、 Availability（可用性）、Partition tolerance（分区容错性），三者不可同时获得，最多只能两个，三者不可兼得。
...
- CAP理论就是说在分布式存储系统中，最多只能实现上面的两点。而由于当前的网络硬件肯定会出现延迟丢包等问题，所以分区容忍性是我们必须需要实现的，而一致性和分区容错性是反比例的，一致性越高对于数据同步的耗时越高相对的响应客户端的时间越长，所以我们只能在一致性和可用性之间进行权衡
...
@EnableEurekaClient
@FeignClient(name = "TestFeign", fallback = TestFallBack.class)可以fallback异常处理
```
...
（1）缓存穿透：穿透，redis和数据库都没有，被整个穿透过去了；
（2）缓存击穿：击穿就是单点集中式访问，描述强大的冲击作用，某个key失效导致；
...
java动态代理是利用反射机制生成一个实现代理接口的匿名类(需要定义接口)，在调用具体方法前调用InvokeHandler来处理。
...
1.定义配置类
2.在META-INF/spring.factories加入配置类
```
...
```
	同步异步：两个任务之间是否存在顺序的依赖关系执行。
	阻塞非租塞：两个任务之间可以同时执行。
	类似于下载任务
	同步 小明下载需要等待下载完成 阻塞下载期间不能执行其他任务
	异步 小明下载不需要等待下载完成 不阻塞下载期间能执行其他任务
...
![img](assets/1099139-20181122234128459-762803724-5052721.png)
...
![img](assets/291012414148876.gif)
...
适配器模式
一种扩展对现有和新加接口的适配 例如对接其它系统字段不匹配的问题 可以使用一个中间类 继承之前和新加的类 一个类扩展实现两个类的功能。增加额外的接口判断增加的业务类型实现原有和新增加业务的兼容。
...
- 5.类似于支付常见跳转微信支付后，前端应将用户支付操作成功之后的支付成功按钮操作动作给后端
  - 类似增加状态用户已提交支付若是再次发起请求直接返回

## [排查OOM](source/java/jvm/排查OOM.md)
